...
namespace SCR
{
	...
	public:
		...
		template<MemSizeLessEqual<MaxSize> T>
	...
	class MultiMemoryPool final
	{
		using MemoryBufferV = MemoryBuffer<Alignment>;
	public:
		MultiMemoryPool()
		...
		MultiMemoryPool(const MultiMemoryPool& aOther)
			: myCurrentBufferIndex(aOther.myCurrentBufferIndex)
		{
			for (const std::unique_ptr<MemoryBufferV>& buffer : aOther.myMemoryBuffers)
			{
				myMemoryBuffers.emplace_back(std::make_unique<MemoryBufferV>(*buffer));
			}
		}


		template<MemSizeLessEqual<Alignment> T>
		T& Allocate(const T& aDefaultValue = T())
		{
			...
			MemoryBufferV& currentBuffer = GetCurrentBuffer();
			return currentBuffer.Allocate(aDefaultValue);
		}
	...
	private:
		...
		void AllocateNewBuffer()
		{
			...
			myMemoryBuffers.emplace_back(std::move(std::make_unique<MemoryBufferV>()));
		}

		MemoryBufferV& GetCurrentBuffer()
		{
			return *myMemoryBuffers.at(myCurrentBufferIndex);
		}

	private:

		std::vector<std::unique_ptr<MemoryBufferV>> myMemoryBuffers;
