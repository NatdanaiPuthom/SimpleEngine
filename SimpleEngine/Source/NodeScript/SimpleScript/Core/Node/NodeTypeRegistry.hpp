...
namespace FLY_NAMESPACE
{
	...
	inline T GetterNode(const InternalExecutionContext* aContext)
	{
		...
		const void* mRuntimeDataPtr = aContext->mClassInstance->mVariableManagerInstance.mVariables[varID].mRuntimeDataPtr;
		const T& output = *reinterpret_cast<const T*>(mRuntimeDataPtr);
		return output;
	}
	...
	inline void SetterNode(const InternalExecutionContext* aContext, const T& aValue)
	{
		...
		T& runtimeValue = *reinterpret_cast<T*>(variableInstance.mRuntimeDataPtr);
		runtimeValue = aValue;
	}
